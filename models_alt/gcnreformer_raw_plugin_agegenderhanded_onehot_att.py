...
class SageReformerLSTMRawPluginAgeGenderHandedOneHotAttFst(torch.nn.Module):
    ...
    def forward_reformer(self, x_in):
        ...
        for i in range(0, n):
            try:
                ...
                with torch.no_grad():
                   log = "Reformer: {}/{}".format(i, n)
                   log = "Max:{}  Min:{}".format(torch.max(x_in), torch.min(x_in))
                   #print(log)
                   # print(log)
