...
from model_tools.positionencoding import PositionalEncoding


class SageReformerLSTMRawPluginAgeGenderHandedOneHotAttSingle(torch.nn.Module):
    def __init__(self, out_channels, lenin=8192,  BS=16):
        ...
        self.fcn_reformer = Linear(in_channels=512, out_channels=640)
        #self.maxlen = 8192
        self.maxlen = 8500
    ...
    def initReformer0(self, embedding_dim=512):
        ''''''
        DE_SEQ_LEN  = 8192
        encoder = ReformerLM(
            num_tokens=20000,
            ...
            ff_dropout=0.1,
            post_attn_dropout=0.1,
            layer_dropout=0.1 # return output of last attention layer
        )
    ...
    def forward(self, x_in, edge_index, gender, age, handed):
        ...
        # embedding_layer = torch.nn.Linear(19*self.BS, embedding_dim)
        # x_embedded = embedding_layer(x_in.to("cpu").T).to("cuda")  # Shape: (8500, 512)
        #pos_encoder = PositionalEncoding(embedding_dim)
        #x_embedded = pos_encoder(x_embedded)  # Shape: (8500, 512)

        x = self.forward_reformer(x_in)
        #x = self.reformer_ap(x_embedded)
        #x = torch.nn.Linear(x, 640)
    ...
    def forward_reformer(self, x_in):
        ...
        with torch.no_grad():
            x = torch.flatten(x_in, start_dim=0)
            x = self.reformer_ap(x)

        x_out = x.type(torch.float32) / m_factor
        return x_out

    def forward_reformer1(self, x_in):
        m_factor, d_factor = 10, 700
        x_in = (x_in+d_factor)*m_factor
        x_in = x_in.type(torch.int32)

        with torch.no_grad():
           x = self.reformer_ap(x_in)

        x_out = x.type(torch.float32) / m_factor
        return x_out

    def forward_reformer0(self, x_in):
        m_factor, d_factor = 10, 700
        x_in = (x_in+d_factor)*m_factor
        x_in = x_in.type(torch.int32)
