...
class GCNLSTMRawPluginAgeGenderHandedOneHot(torch.nn.Module):
    ...
    def forward(self, x_in, edge_index, edge_attr,  gender, age, handed):
        ...
        x = self.gcn4(x, edge_index, edge_attr)
        x = F.leaky_relu(x)
        x = F.batch_norm(x, None, None, training=True)
    ...
    def getLabel(self):
        lbl = "GCNLSTMRawPluginAgeGenderHandedOneHot"
        return lbl