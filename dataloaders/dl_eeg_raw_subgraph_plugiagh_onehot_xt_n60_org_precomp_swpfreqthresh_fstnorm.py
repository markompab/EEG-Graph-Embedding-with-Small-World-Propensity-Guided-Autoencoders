...
class DLSubGPlgGAHNorm60OrgPSIAllSWPFreqThreshPreFstNorm(Dataset):
    ...
    def genNetxGraph(self, nodes, edges, edge_weight, pos):
    ...
    def getBstSWPGraph(self, srcpathswpcorr):
        ...
        dt = np.load(srcpathswpcorr, allow_pickle=True)
        edge_index, edge_attr = torch.tensor(dt["edge_index"]), torch.tensor(dt["edge_attr"])

        nds = torch.unique(edge_index)
